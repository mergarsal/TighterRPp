...
PrimalBoth::PrimalBoth(const Matrix9& C, const bool debug)
{ 
  ...
  dimT_ = 3;
  dimQ_ = 3; 
  nConstraints_ = 7 + 6; // 7 for E E^T and 6 for E^T E
  int nBlock = 3;    // number blocks: E; t; q
  ...
  // if (debug)  Problem1_.printParameters(stdout);

  if (debug) std::cout << "[SDP-BOTH] Create type solution\n";
  ...
  Problem1_.inputBlockSize(3,dimQ_);
  Problem1_.inputBlockType(1,SDPA::SDP);
  Problem1_.inputBlockType(2,SDPA::SDP);
  Problem1_.inputBlockType(3,SDPA::SDP);

  Problem1_.initializeUpperTriangleSpace();
  
  if (debug) std::cout << "[SDP-BOTH] Define vector b\n";
  //cVECT = {1,1, ..., 0}
  ...
  if (debug) std::cout << "[SDP-BOTH] Define data matrix\n";
  ...
  for (int i = 1; i <= 9; i++)
  {
        for (int j = 1; j <= 9; j++)
        {
                 ...
                 // std::cout << "Element (i, j): " << i << "," << j << " = " << C(i - 1,j - 1) << std::endl;
   ...
   for (int i = 1; i <= 3; i++)
  {
        ...
        {
                 // NOTE: SDPA consider the problem as MAX
                 Problem1_.inputElement(0, 2, i, j, 0 ); 
                 Problem1_.inputElement(0, 3, i, j, 0 ); 
        }
  ...
  int t1=1,t2=2,t3=3,q1=1,q2=2,q3=3;
  
  // Norm t
  if (debug) std::cout << "[SDP-BOTH] Define constraint norm t\n";
  // 2nd block
  ...
  // 3rd block
  Problem1_.inputElement(2, 3, q1, q1, 1);
  Problem1_.inputElement(2, 3, q2, q2, 1);
  Problem1_.inputElement(2, 3, q3, q3, 1); 
  
  
  if (debug) std::cout << "[SDP-BOTH] Define constraint E*E^T\n";
  // ---------  Input F_2  --------------------
  ...
  Problem1_.inputElement(id_const, 1, e6, e6, 1);
  Problem1_.inputElement(id_const, 1, e9, e9, 1);
  Problem1_.inputElement(id_const, 3, q1, q1, -1);
  Problem1_.inputElement(id_const, 3, q2, q2, -1);
  ...
  Problem1_.inputElement(id_const, 1, e8, e8, 1);
  Problem1_.inputElement(id_const, 3, q1, q1, -1);
  Problem1_.inputElement(id_const, 3, q3, q3, -1);
  ...
  */
  ...
  Problem1_.inputElement(id_const, 3, q1, q2, 1);
  ...
  Problem1_.inputElement(id_const, 3, q1, q3, 1);
  ...
  Problem1_.inputElement(id_const, 3, q2, q3, 1);
...
PrimalBothResult PrimalBoth::getResult(bool debug){


  /* Solve problem now */ 
  if (debug) std::cout << "[SDP-BOTH] Getting result\n";
  Problem1_.initializeUpperTriangle();
  if (debug) std::cout << "[SDP-BOTH] Init solver\n";
  Problem1_.initializeSolve();
  
  // if necessary, dump input data and initial point
  if (debug) std::cout << "[SDP-BOTH] Solve!\n";
  Problem1_.solve();
  
  //
  if (debug) std::cout << "[SDP-BOTH] Saving results\n";
  PrimalBothResult res = PrimalBothResult(); 
  ...
  if (debug) std::cout << "[SDP-BOTH] Saving lagrange multipliers\n";
  ...
  for (int i=0; i<dimT_; ++i) {
   ...
   for (int i=0; i<dimT_; ++i) {
    for (int j=0; j<dimT_; ++j) {
      res.primal_Xt(i+3,j+3) = Xq[i * dimT_ + j];  // 3 x 3
      }
      }  
...
void PrimalBoth::getSolutionFromResult(PrimalBothResult & res, bool debug){
    ...
    auto max_index = [] (const Eigen::Matrix<double, Eigen::Dynamic, 1> & e_vector, 
       ...
       if (debug)  std::cout << "Extracting data from SDP solution\n"; 
        ...
        double val_rot = eigenvalues_Xe.maxCoeff();  
                ...
                // std::cout << "Vectorized E:\n" << expanded_e.normalized() << std::endl; 
