...
PrimalRight::PrimalRight(const Matrix9& C, const bool debug)
{ 
  ...
  if (debug) std::cout << "[SDP-RIGHT] Create type solution\n";
  ...
  if (debug) std::cout << "[SDP-RIGHT] Define vector b\n";
  //cVECT = {1,0, ..., 0}
  ...
  if (debug) std::cout << "[SDP-RIGHT] Define data matrix\n";
  // 1st block: C
  for (int i = 1; i <= 9; i++)
  {
        for (int j = i; j <= 9; j++)
  ...
  // Norm t
  if (debug) std::cout << "[SDP-RIGHT] Define constraint norm t\n";
  // 2nd block
  // # constraint, block, i1, j1, value
  Problem1_.inputElement(1, 2, q1, q1, 1);
  Problem1_.inputElement(1, 2, q2, q2, 1);
  Problem1_.inputElement(1, 2, q3, q3, 1); 
 
  
  
  
  if (debug) std::cout << "[SDP-RIGHT] Define constraint E*E^T\n";
  // ---------  Input F_2  --------------------
...
PrimalRightResult PrimalRight::getResult(bool debug){


  /* Solve problem now */ 
  if (debug) std::cout << "[SDP-RIGHT] Getting result\n";
  Problem1_.initializeUpperTriangle();
  if (debug) std::cout << "[SDP-RIGHT] Init solver\n";
  Problem1_.initializeSolve();
  
  // if necessary, dump input data and initial point
  if (debug) std::cout << "[SDP-RIGHT] Solve!\n";
  Problem1_.solve();
  
  //
  if (debug) std::cout << "[SDP-RIGHT] Saving results\n";
  PrimalRightResult res = PrimalRightResult(); 
  ...
  if (debug) std::cout << "[SDP-RIGHT] Saving lagrange multipliers\n";
  ...
  if (debug) std::cout << "[SDP-RIGHT] Saving blocks for solutions\n";
...
void PrimalRight::getSolutionFromResult(PrimalRightResult & res, bool debug){
